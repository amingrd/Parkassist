    ...
    <main class="dashboard-stack">
      ...
      <section class="content-stack">
        <div class="tab-row">
          ...
          <a class="auth-tab" href="/banners">Banners</a>
          {admin_link}
        </div>
        {panel}
      </section>
...
def banners_page(
    *,
    ...
    selected_config = selected_template["config"]
    options = "".join(
        f"<option value='{escape(template['id'])}' {'selected' if template['id'] == selected_template['id'] else ''}>{escape(template['display_name'])}</option>"
    ...
    if latest_run:
        latest_html = f"""
        <section class="glass panel-card">
          <p class="eyebrow">Latest export</p>
          <h3>{escape(latest_run['template_display_name'])}</h3>
          <p class="lead compact">Created {escape(latest_run['created_at'])}. Download the ZIP package or re-run the same values from history.</p>
          <a class="nav-button inline-link" href="/banners/download/{latest_run['id']}">Download ZIP</a>
        </section>
        """
    ...
    history_cards = "".join(
        ...
        <tr>
          ...
          <td>{escape(run['template_display_name'])}</td>
          <td>{escape(run['headline'])}</td>
          <td>{escape(run['status'])}</td>
          <td>{escape(run['original_image_name'])}</td>
          <td class="history-actions">
            {"<a class='nav-button inline-link' href='/banners/download/" + str(run['id']) + "'>Download</a>" if run['status'] == 'completed' else ""}
            <form method="post" action="/banners/{run['id']}/rerun" class="inline-form">
              <button class="ghost-button" type="submit">Re-run</button>
            </form>
          </td>
        </tr>
        """
    ...
    <main class="dashboard-stack">
      <section class="glass week-panel admin-hero">
        <div class="week-header">
          <div>
            <p class="eyebrow">Internal banners</p>
            <h2 class="week-range">Figma-ready banner exports</h2>
            <p class="lead compact">Create controlled banner variants with text limits, image validation, export packaging, and recent-history re-runs.</p>
          </div>
          <div class="week-nav">
            <a class="nav-button" href="/?tab=booking">Back to booking view</a>
            <a class="nav-button" href="{escape(selected_template['figma_file_url'])}" target="_blank" rel="noreferrer">Open Figma file</a>
          </div>
        </div>
      </section>
      <section class="banner-layout">
        <section class="glass panel-card">
          <p class="eyebrow">Create export</p>
          <h3>{escape(selected_template['display_name'])}</h3>
          <form method="post" action="/banners/generate" enctype="multipart/form-data" class="stack-form">
            <label>
              Template set
              <select name="template_set_id">{options}</select>
            </label>
            <label>
              Headline
              <input type="text" name="headline" value="{escape(draft_values.get('headline', ''))}" maxlength="{selected_config['headline_max_chars']}" data-char-count="headline-count" required>
              <span class="helper-text banner-meta"><strong id="headline-count">{len(draft_values.get('headline', ''))}</strong> / {selected_config['headline_max_chars']} characters</span>
              {"<span class='form-error'>" + escape(field_errors['headline']) + "</span>" if field_errors.get('headline') else ""}
            </label>
            <label>
              Subline
              <input type="text" name="subline" value="{escape(draft_values.get('subline', ''))}" maxlength="{selected_config['subline_max_chars']}" data-char-count="subline-count" required>
              <span class="helper-text banner-meta"><strong id="subline-count">{len(draft_values.get('subline', ''))}</strong> / {selected_config['subline_max_chars']} characters</span>
              {"<span class='form-error'>" + escape(field_errors['subline']) + "</span>" if field_errors.get('subline') else ""}
            </label>
            <label>
              Button text
              <input type="text" name="button_text" value="{escape(draft_values.get('button_text', ''))}" maxlength="{selected_config['button_text_max_chars']}" data-char-count="button-count" required>
              <span class="helper-text banner-meta"><strong id="button-count">{len(draft_values.get('button_text', ''))}</strong> / {selected_config['button_text_max_chars']} characters</span>
              {"<span class='form-error'>" + escape(field_errors['button_text']) + "</span>" if field_errors.get('button_text') else ""}
            </label>
            <label>
              Image upload
              <input type="file" name="image_upload" accept=".png,.jpg,.jpeg,.webp" required>
              <span class="helper-text banner-meta">Allowed: PNG, JPEG, WebP. Minimum {selected_config['min_width_px']} x {selected_config['min_height_px']} px. Maximum {selected_config['max_file_size_bytes'] // (1024 * 1024)} MB.</span>
              {"<span class='form-error'>" + escape(field_errors['image_upload']) + "</span>" if field_errors.get('image_upload') else ""}
            </label>
            <button type="submit">Generate banner ZIP</button>
          </form>
        </section>
        <section class="banner-sidebar">
          <section class="glass panel-card">
            <p class="eyebrow">Template details</p>
            <h3>Exported variants</h3>
            <ul class="rule-list">
              {"".join("<li>" + escape(variant['name']) + " - " + escape(str(variant['width'])) + " x " + escape(str(variant['height'])) + "</li>" for variant in selected_config['variants'])}
            </ul>
            <p class="helper-text">Figma file key: {escape(selected_template['figma_file_key'])}</p>
          </section>
          {latest_html}
        </section>
      </section>
      <section class="glass panel-card">
        ...
        <h3>Your recent banner runs</h3>
        <table>
          <thead><tr><th>Created</th><th>Template</th><th>Headline</th><th>Status</th><th>Image</th><th>Actions</th></tr></thead>
          <tbody>{history_cards or "<tr><td colspan='6'>No banner exports yet.</td></tr>"}</tbody>
        </table>
      </section>
    </main>
    """
    return html_page("Banner Tool", body, current_user=current_user, flash=flash)
